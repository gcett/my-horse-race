...
watch(() => horseStore.raceSchedule, () => {
  Object.keys(positions).forEach(id => {
    positions[id] = 0
  })
})
const currentRound = computed(() => {
  return horseStore.raceSchedule[currentRoundIndex.value] || null
})

function startRace() {
  if (!horseStore.raceSchedule.length) return
...
function animateLap() {
  ...
  // 2. Kısa bir bekleme sonra yarış başlasın
  setTimeout(() => {
    animationTimer = setInterval(() => {
      let allFinished = true

      currentRound.value.horses.forEach(horse => {
        const speed = 0.3 + horse.condition / 1000  // Daha sakin hareket
        if (positions[horse.id] < 100) {
          positions[horse.id] += speed
          allFinished = false
        }
      ...
      if (allFinished) {
        ...
        // Eğer son round değilse → 1 sn sonra yeni tur
        if (currentRoundIndex.value < horseStore.raceSchedule.length - 1) {
          setTimeout(() => {
          ...
          // Sonuçları oluştur
          const allResults = horseStore.raceSchedule.map((round, index) => {
            const sorted = [...round.horses]
                .map(h => ({ ...h, score: h.condition }))
                .sort((a, b) => b.score - a.score)

            return {
              lap: index + 1,
              distance: round.distance,
              horses: sorted.map(({ score, ...rest }) => rest)
            }
          })

          horseStore.setRaceResults(allResults)
        }
      }
    }, 30) // her 30ms'de güncelleme — daha yumuşak animasyon
  }, 500) // 1 saniye beklemeden sonra tur başlasın
}
</script>
...
<template>
  <div class="track-container">
    ...
    <p class="lap-info" v-if="currentRound">
      {{ currentRoundIndex + 1 }}. Lap - {{ currentRound.distance }}m
    </p>
    <p class="lap-info" v-else-if="!horseStore.isRunning && horseStore.raceSchedule.length">
      Race Finished 🏁
    </p>
    ...
    <div class="track">
      ...
      <template v-if="currentRound">
        ...
        >
          ...
          <div
              ...
              :style="{
              color: horse.color,
              left: `calc(36px + ${(positions[horse.id] || 0)}%)`,
      ...
      <template v-else>
        <p class="no-program">Please click "Generate Program" to start.</p>
      </template>
    </div>
  </div>
</template>
...
.lane {
  ...
  align-items: center;
  gap: 12px;
  border-bottom: 2px dashed #aeaaaa;
  padding: 0 0;
  position: relative;
  min-height: 30px;
}
